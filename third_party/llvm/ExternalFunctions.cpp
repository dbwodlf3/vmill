 ...
 //  VmillInterpreter.  The first is to implement lle_* wrapper functions that are
 ...
 //  not exist, and libffi is available, then the VmillInterpreter will attempt to
 ...
 static VmillInterpreter *TheInterpreter;
 ...
 static bool ffiInvoke(RawFunc Fn, Function *F, ArrayRef<GenericValue> ArgVals,
   ...
   if (ArgVals.size() > NumArgs && F->isVarArg()) {
     report_fatal_error("Calling external var arg function '" + F->getName()
                       + "' is not supported by the VmillInterpreter.");
   }
 ...
 GenericValue VmillInterpreter::callExternalFunction(Function *F,
 ...
 void VmillInterpreter::initializeExternalFunctions() {
   sys::ScopedLock Writer(*FunctionsLock);

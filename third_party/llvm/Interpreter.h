...
class VmillAllocaHolder {
 ...
 public:
  ...
  VmillAllocaHolder(VmillAllocaHolder &&) = default;
  VmillAllocaHolder &operator=(VmillAllocaHolder &&RHS) = default;
...
// VmillExecutionContext struct - This struct represents one stack frame currently
// executing.
//
struct VmillExecutionContext {
  ...
  VmillAllocaHolder Allocas;            // Track memory allocated by alloca
...
//
class VmillInterpreter : public ExecutionEngine, public InstVisitor<
    VmillInterpreter> {
 protected:
  //monkey patch so I can access values for a custom runFunction method that interacts with handler
 ...
 public:
  ...
  ///
  GenericValue runFunction(Function *F, ArrayRef<GenericValue> ArgValues)
      override;

  void *getPointerToNamedFunction(StringRef Name, bool AbortOnFailure = true)
      override {
 ...
 private:
  ...
  // control flow.
  //
  void SwitchToNewBasicBlock(BasicBlock *Dest, VmillExecutionContext &SF);
  ...
  GenericValue executeCastOperation(Instruction::CastOps opcode, Value *SrcVal,
                                    Type *Ty, VmillExecutionContext &SF);
